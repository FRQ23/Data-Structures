...
ExprNode* createNode(char op, double value, char var);
...
void imprimirArbol(ExprNode* nodo, int nivel);
...
ExprNode* buildExpressionTree(char *expr, double vars[NUM_VARIABLES]) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (isalpha(expr[i])) {
            push(&values, createNode('\0', vars[expr[i] - 'a'], expr[i]));
            ...
            i--;
            push(&values, createNode('\0', val, '\0'));
        } else if (expr[i] == '(') {
            push(&ops, createNode(expr[i], 0, '\0'));
        } else if (expr[i] == ')') {
    ...
    while (1) {
        ...
        switch (opcion) {
            case 0:
                ...
                root = buildExpressionTree(expr, vars);
                if (root) {
            ...
            case 2:
                ...
                if (root) freeExpressionTree(root);
                return 0;
            default:
