...
Quiero implementar una cola auxiliar para respetar fifo en las funciones enqueue y dequeue, se que el código respeta fifo, pero sé que el usuario puede eliminar en la posición que quiera asi como agregar, pero independientemente que el usuario pueda hacer eso, yo quiero generar una cola auxiliar, para respetar ese fifo, al final la cola auxiliar se hará la nueva cola, asi se respeta el fifo. Tanto cuando se agregue como se eliminen elementos de la cola
...
void inicializarColaAuxiliar(ColaAuxiliar* colaAux);
